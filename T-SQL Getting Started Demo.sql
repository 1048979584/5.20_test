...
DECLARE @string_a CHAR(5),@string_b VARCHAR(5),
		@string_c NCHAR(5),@string_d NVARCHAR(5)
...
美蛋：SQL_Latin1_General_CP1_CI_AS(CI:case-insensitive大小写不敏感，AS：accent-sensitive重音敏感，)
中蛋：Chinese_PRC_CI_AS
1.因为在存储varchar字段时，从底层数据行的结构，它会比char多维护2个字节（可变长度字段的列偏移阵列）,
所以基于性能及维护成本，我们赞成字符数小于3的都是用char,nchar来定义。
数据类型CHAR/NCHAR当宽度超过30，请考虑使用VARCHAR/NVARCHAR代替,除非存储的值是定长的.
2.xml/varhcar(max)/nvarchar(max)这三种类型的列，
建议存储在独立的表中，否则会产生很大的性能问题
3.char,varchar类型字段，需要预估是否包含多国字符，如果是，请使用nchar,nvarchar
4.在 Microsoft SQL Server 的未来版本中将删除 ntext、text 和 image 数据类型。请避免在新开发工作中使用这些数据类型，
并考虑修改当前已使用这些数据类型的应用程序，请改用 nvarchar(max)、varchar(max) 和 varbinary(max)，
即使是在临时表，表变量也不允许使用
*/

--Money类型是不允许使用的，请用Decimal代替
...
一致性(Consistency）：一致性阐明了在成功地完成一个事务之后，所有的数据都处于一致状态。
...
持久性(Durability)：持久性阐明了完整的事务对数据的任何修改能够在系统中永久保持其效果。
...
*/
/* 
1.XACT_ABORT选项的设置，系统默认为OFF
2.处理事务时，尽量使用TRY...CATCH来做异常捕获及处理
...
BEGIN TRY
	...
	INSERT INTO dbo.t1 VALUES (1); -- Foreign key error.
...
CREATE TABLE rx66.[Contacts](
	...
	CONSTRAINT DF_Contacts_ModifiedDate DEFAULT GETDATE()
...
,rowguid
,[ModifiedDate]
FROM sm98.Contacts WITH(NOLOCK)
...
EXEC @ret = dbo.[UF_ufnGetSalesOrderStatusText] @Status= 5; 
PRINT @ret;
...
SELECT item,manufactory,ItemCatalog,CurrentPrice FROM dbo.arinvt01 WITH(NOLOCK) 
WHERE manufactory=1213 AND ItemCatalog='349'
...
SELECT @datebegin='2017-10-13 06:40:00',@dateend='2017-10-13  07:40:00'

--SET STATISTICS PROFILE ON

SELECT a.TableName,
SUM(total_write) AS Total_write ,
SUM(total_read) AS Total_read
,SUM(total_request) AS Total_request  
FROM 
(
	SELECT --RM.ClusterName,
		RM.TableName
			--RM.RegionName
		,SUM(RMD.[writeRequestsCountDiff]) AS Total_write
		,SUM(RMD.[readRequestsCountDiff]) AS Total_read
		,SUM(RMD.[requestsCountDiff]) AS Total_request
	FROM [BigDataMonitor].[dbo].[RegionMetricsDifference] AS RMD WITH(NOLOCK)
	INNER JOIN RegionMetrics AS RM WITH(NOLOCK)
	ON  RMD.rmRowId=RM.rowid AND RMD.BatchId=RM.BatchId
	WHERE RMD.BatchId IN (SELECT  BatchId FROM dbo.tb_RecordDate WITH(NOLOCK) WHERE monitorType='RegionMetrics' and indate>@datebegin and indate<@dateend
						  UNION ALL 
						  SELECT BatchId FROM dbo.tb_RecordDate_archive WITH(NOLOCK) WHERE monitorType='RegionMetrics' and indate>@datebegin and indate<@dateend
						) 
		 AND RM.ClusterName='E4ECOFL' --and RM.TableName ='ecitem:EC_ItemReviewSolr'
	GROUP BY RM.TableName--,RM.RegionName
	UNION ALL
	SELECT --RM.ClusterName,
		RM.TableName
	--RM.RegionName
		...
		,SUM(RMD.[readRequestsCountDiff]) AS Total_read
	    ,SUM(RMD.[requestsCountDiff]) AS Total_request
	FROM [BigDataMonitor].[dbo].[RegionMetricsDifference_archive] AS RMD WITH(NOLOCK)
	INNER JOIN RegionMetrics_archive AS RM WITH(NOLOCK)
	ON  RMD.rmRowId=RM.rowid AND RMD.BatchId=RM.BatchId
	WHERE RMD.BatchId IN (SELECT BatchId FROM dbo.tb_RecordDate WITH(NOLOCK) WHERE monitorType='RegionMetrics' and indate>@datebegin and indate<@dateend
						   UNION ALL  
						  SELECT BatchId FROM dbo.tb_RecordDate_archive WITH(NOLOCK) WHERE monitorType='RegionMetrics' and indate>@datebegin and indate<@dateend
...
/*exists根据子查询中的每一行决定返回True或者False,当遇到NULL值时,只返回False,
而不会由某个Null值导致整个子查询表达式为NULL.*/
...
/*即使productID中没有NULL值，但是由于该列定义允许NULL值，所以SQL Server并不知道有没有NULL值，
需要先判断表中是否有NULL值（row count spool和index seek）。*/
...
-- AND NOT EXISTS(SELECT ProductID FROM Sales.SalesOrderDetail AS SOD1 WHERE SOD1.ProductID IS NULL )
...
SET @top=2
SET @sql=N'SELECT TOP '+CAST(@top AS NVARCHAR(4))+
		   N' name,object_id,type,type_desc,create_date,modify_date 
...
SET @TableName=N'product'
SET @top=2
SET @sql=N'SELECT TOP (@top) name,object_id,type,type_desc
...
SET @sql=N'
SELECT top 5 name,object_id,type,type_desc
FROM sys.tables WITH (NOLOCK)
WHERE 1=1'
...
BEGIN
  SET @sql=@sql+' AND create_date>='''+CONVERT(CHAR(20),@crdate,120)+''''
END
...
IF @crdate IS NOT NULL
BEGIN
    SET @sql=@sql+' AND create_date>=@crdate'
END
...
SELECT @id='1'
SELECT @sql1=N'SELECT top(1) name,log_reuse_wait_desc 
			from sys.databases with(nolock) where database_id='+@id+''
EXEC (@sql1)
...
SELECT @sql2=N'SELECT top(1) name,log_reuse_wait_desc  
		   from sys.databases with(nolock) where database_id=@id'
...
where t.text like '%log_reuse_wait_desc%'   
